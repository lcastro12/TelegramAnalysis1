...
public class UpdateFragment extends DialogFragment implements OnClickListener, UpdateInfoListener {
    ...
    class C00532 extends DownloadFileListener {
        C00532() {
        }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        TextView nameLabel = (TextView) view.findViewById(C0051R.id.label_title);
        nameLabel.setText(Util.getAppName(getActivity()));
        nameLabel.setContentDescription(nameLabel.getText());
        TextView versionLabel = (TextView) view.findViewById(C0051R.id.label_version);
        String versionString = String.format(getString(C0051R.string.hockeyapp_update_version), new Object[]{versionHelper.getVersionString()});
        final String fileDate = versionHelper.getFileDateString();
        String appSizeString = getString(C0051R.string.hockeyapp_update_unknown_size);
            ...
            AsyncTaskUtils.execute(new GetFileSizeTask(getActivity(), this.mUrlString, new DownloadFileListener() {
                public void downloadSuccessful(DownloadFileTask task) {
                    if (task instanceof GetFileSizeTask) {
                        ...
                        textView.setText(UpdateFragment.this.getString(C0051R.string.hockeyapp_update_version_details_label, new Object[]{str, fileDate, appSizeString}));
                    }
                }
            }));
        }
        versionLabel.setText(getString(C0051R.string.hockeyapp_update_version_details_label, new Object[]{versionString, fileDate, appSizeString}));
        ((Button) view.findViewById(C0051R.id.button_update)).setOnClickListener(this);
        WebView webView = (WebView) view.findViewById(C0051R.id.web_update_details);
    ...
    private void showError(int message) {
        new Builder(getActivity()).setTitle(C0051R.string.hockeyapp_dialog_error_title).setMessage(message).setCancelable(false).setPositiveButton(C0051R.string.hockeyapp_dialog_positive_button, null).create().show();
    }
    ...
    protected void prepareDownload() {
        ...
        if (!Util.isConnectedToNetwork(context)) {
            showError(C0051R.string.hockeyapp_error_no_network_message);
        } else if (!PermissionsUtil.permissionsAreGranted(PermissionsUtil.permissionsState(context, requiredPermissions()))) {
            showError(C0051R.string.hockeyapp_error_no_external_storage_permission);
            ...
            showError(C0051R.string.hockeyapp_error_install_form_unknown_sources_disabled);
        }
    }

    protected void startDownloadTask() {
        AsyncTaskUtils.execute(new DownloadFileTask(getActivity(), this.mUrlString, new C00532()));
    }

    public View getLayoutView() {
        LinearLayout layout = new LinearLayout(getActivity());
        LayoutInflater.from(getActivity()).inflate(C0051R.layout.hockeyapp_fragment_update, layout);
        return layout;
    }
}
