...
public final class SsMediaSource implements MediaSource, Callback<ParsingLoadable<SsManifest>> {
    ...
    private void processManifest() {
        ...
        if (startTimeUs == Long.MAX_VALUE) {
            timeline = new SinglePeriodTimeline(this.manifest.isLive ? C0600C.TIME_UNSET : 0, 0, 0, 0, true, this.manifest.isLive);
            startTimeUs2 = startTimeUs;
        } else if (this.manifest.isLive) {
            if (this.manifest.dvrWindowLengthUs == C0600C.TIME_UNSET || this.manifest.dvrWindowLengthUs <= 0) {
                startTimeUs2 = startTimeUs;
            } else {
                startTimeUs2 = Math.max(startTimeUs, endTimeUs - this.manifest.dvrWindowLengthUs);
            }
            durationUs = endTimeUs - startTimeUs2;
            long defaultStartPositionUs = durationUs - C0600C.msToUs(this.livePresentationDelayMs);
            ...
            Timeline singlePeriodTimeline = new SinglePeriodTimeline(C0600C.TIME_UNSET, durationUs, startTimeUs2, defaultStartPositionUs, true, true);
        } else {
            durationUs = this.manifest.durationUs != C0600C.TIME_UNSET ? this.manifest.durationUs : endTimeUs - startTimeUs;
